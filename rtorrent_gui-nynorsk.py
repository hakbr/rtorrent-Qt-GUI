...
def ssh_subprocess_env(cfg):
    ...
    if not password:
        raise RuntimeError(
            "Passordautentisering er valt, men det er ikkje lagra noko passord. "
            "Set eitt i Innstillingar."
        )
    env = os.environ.copy()
...
def check_password_auth_ready(cfg):
    ...
    if not sshpass_available():
        raise RuntimeError(
            "Passordautentisering krev verktøyet 'sshpass', som ikkje er "
            ...
            for item in self.items:
                ...
                if self.delete_files:
                    if not base_path.startswith("/") or base_path in ("/", ""):
                        raise RuntimeError(
                            f"Nektar å slette '{name}': rtorrent rapporterte ikkje ein "
                ...
                if self.delete_files:
                    ...
                    if result.returncode != 0:
                        raise RuntimeError(
                            f"Fjerna '{name}' frå rtorrent, men sletting av filene feila: "
                            f"{result.stderr.strip() or 'ukjend feil'}"
                        )
            self.finished_ok.emit()
        except Exception as e:
            self.error.emit(str(e))
...
class SettingsDialog(QDialog):
    def __init__(self, parent, cfg):
        ...
        auth_note_text = (
            "Passordet vert lagra i nøkkelringen til operativsystemet via pakken 'keyring'."
            if KEYRING_AVAILABLE else
            "MERK: pakken 'keyring' er ikkje installert, så passordet vert lagra "
            "i klartekst i config.json (installer keyring med pip install keyring, "
            "eller apt install python3-keyring, for å lagre det trygt i staden)."
        )
        self.auth_note_label = QLabel(auth_note_text)
        self.auth_note_label.setWordWrap(True)
        self.sshpass_note_label = QLabel(
            "MERK: passordautentisering krev òg verktøyet 'sshpass' "
        ...
        self.download_dir_edit.setPlaceholderText(
            "~/downloads (lat stå tomt for å bruke rtorrent sin eigen standardmappe)"
        )
        ...
        form.addRow("Fjernsti til socket:", self.remote_socket_edit)
        form.addRow(QLabel("<b>rtorrent RPC (TCP, berre viss du ikkje brukar unix-socket)</b>"))
        form.addRow("Fjernvert (bind):", self.remote_host_edit)
        form.addRow("Fjernport:", self.remote_port_spin)
        form.addRow(QLabel("<b>Avspørring</b>"))
        form.addRow("Oppdater kvar:", self.poll_spin)
...
COLUMNS = [
    "Namn", "Status", "Framgang", "Nedlastingsfart", "Opplastingsfart",
    "Attverande tid", "Jamningar", "Forhold", "Storleik", "Lasta ned", "Lasta opp",
]
...
TRACKER_COLUMNS = ["URL", "Type", "Aktivert", "Delarar", "Mottakarar"]
PEER_COLUMNS = ["Adresse", "Klient", "Nedlastingsfart", "Opplastingsfart", "Framgang", "Kryptert"]
...
class TrackerPeerDialog(QDialog):
    ...
    def __init__(self, parent, rpc, torrent_hash, torrent_name, poll_interval):
        super().__init__(parent)
        self.setWindowTitle(f"Trackar- og jamningsinfo — {torrent_name}")
        self.resize(720, 480)
        self.rpc = rpc
        self.torrent_hash = torrent_hash
        self.worker = None
        ...
        layout.addWidget(QLabel("<b>Trackarar</b>"))
        ...
        layout.addWidget(QLabel("<b>Jamningar</b>"))
    ...
    def on_result(self, trackers, peers):
        self.status_label.setText(f"{len(trackers)} trackar(ar), {len(peers)} jamning(ar)")
...
class MainWindow(QMainWindow):
    ...
    def connect_all(self):
        ...
        if not cfg["ssh_host"] or not cfg["ssh_user"]:
            QMessageBox.warning(self, "Manglar innstillingar", "Fyll ut SSH-vert og brukarnamn i Innstillingar først.")
            self.open_settings()
            return
        if cfg["mode"] == "unix" and not cfg["remote_socket_path"]:
            QMessageBox.warning(self, "Manglar innstillingar", "Set fjernstien til socket i Innstillingar.")
            self.open_settings()
            return
        try:
            check_password_auth_ready(cfg)
        except RuntimeError as e:
            QMessageBox.warning(self, "Passordautentisering ikkje klar", str(e))
            self.open_settings()
            return
        ...
        if not ready:
            ...
            self.tunnel = None
            if not output:
                output = ("(inga utskrift fanga opp — tunnelen kom aldri opp innan "
                           "tidsgrensa, men ssh rapporterte heller ikkje nokon feil. Dette kan "
                           "tyde på at han framleis forhandlar, eller at ein gamal tunnelprosess "
                           "frå ein tidlegare køyring held på den lokale socketen/porten. Prøv å "
                           "køyre kommandoen under for hand i eit terminalvindauge for å sjå kva "
                           "som eigentleg skjer.)")
            QMessageBox.warning(
                self, "Tilkopling feila",
                "Kunne ikkje opprette SSH-tunnelen til RPC-socketen til rtorrent.\n\n"
                "Sjekk: SSH-nøkkelautentisering fungerer for denne verten, rtorrent "
                "køyrer med RPC aktivert, og fjernstien til socket/port er rett.\n\n"
                f"Kommando køyrd:\n{command}\n\n"
                f"ssh-utskrift:\n{output}"
            )
            self.update_connect_ui(connected=False)
    ...
    def on_tunnel_finished(self, exit_code, exit_status):
        ...
        self.handle_connection_lost(
            "SSH-tunnelen avslutta uventa — kopla ser ut til å ha vorte broten "
            "i andre enden (t.d. eit nettverksbyte, at maskina gjekk i kvile, "
            "eller at tenaren vart starta på nytt).",
            output,
        )
    ...
    def handle_connection_lost(self, reason, detail=""):
        ...
        self.down_limit_spin.setEnabled(False)
        self.up_limit_spin.setEnabled(False)
        self.status_bar.showMessage("Kopling tapt", 5000)
        msg = reason
        if detail:
            msg += f"\n\nssh-utskrift:\n{detail}"
        msg += "\n\nTrykk Kople til for å kople til igjen."
        QMessageBox.warning(self, "Kopling tapt", msg)
    ...
    def on_disk_space_error(self, msg):
        self.disk_space_label.setText("Ledig: ukjend")
        self.disk_space_label.setToolTip(f"Kunne ikkje lese diskplass: {msg}")
    ...
    def on_global_limits_error(self, msg):
        self.status_bar.showMessage(f"Kunne ikkje lese fartsgrenser: {msg}", 5000)
    ...
    def on_limit_set(self, direction, bytes_per_sec):
        label = "Nedlasting" if direction == "down" else "Opplasting"
        shown = "uavgrensa" if bytes_per_sec == 0 else human_speed(bytes_per_sec)
        self.status_bar.showMessage(f"{label}sgrensa sett til {shown}", 4000)

    def on_limit_set_error(self, msg):
        self.status_bar.showMessage(f"Innstilling av fartsgrense feila: {msg}", 6000)

    def on_poll_error(self, msg):
        self.poll_fail_count += 1
        self.status_bar.showMessage(f"Avspørringsfeil: {msg}", 5000)
        if self.poll_fail_count >= 3:
            self.handle_connection_lost(
                "Mista kontakt med rtorrent over tunnelen — kopla ser ut til å ha "
                "vorte broten i andre enden, sjølv om ssh-prosessen framleis "
                "køyrer lokalt (t.d. etter kvile eller eit nettverksbyte).",
                msg,
            )
    ...
    def notify_download_complete(self, name):
        ...
        if QSystemTrayIcon.isSystemTrayAvailable() and self.tray_icon.isVisible():
            self.tray_icon.showMessage(
                "Nedlasting fullført", name, QSystemTrayIcon.Information, 6000
            )
    ...
    def show_context_menu(self, pos):
        ...
        info_action = None
        if len(hashes) == 1:
            info_action = menu.addAction("Trackar- og jamningsinfo...")
            menu.addSeparator()
    ...
    def erase_torrents(self, hashes, delete_files):
        ...
        if delete_files:
            ...
            text = (
                "Dette slettar dei nedlasta filene frå tenaren for godt, i tillegg til "
                "å fjerne torrenten(-ane) frå rtorrent.\n\n"
            ...
            text = (
                "Dette fjernar torrenten(-ane) frå lista til rtorrent. Dei nedlasta "
                "filene vert verande på disken.\n\n"
                f"{names}"
            )
    ...
    def add_magnet(self):
        ...
        if not magnet.startswith("magnet:"):
            QMessageBox.warning(self, "Ugyldig magnetlenke", "Skriv inn ei gyldig magnet:-lenke.")
            return
    ...
    def on_magnet_add_error(self, msg):
        self.status_bar.showMessage(f"Legging til torrent feila: {msg}", 6000)
        QMessageBox.warning(self, "Legging til torrent feila", msg)
    ...
    def add_torrent_file(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Kunne ikkje lese fila", str(e))
            return
    ...
    def on_torrent_file_add_error(self, msg):
        self.status_bar.showMessage(f"Legging til torrent-fil feila: {msg}", 6000)
        QMessageBox.warning(self, "Legging til torrent-fil feila", msg)
